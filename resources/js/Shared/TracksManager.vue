    ...
    <div class="max-w-4xl bg-white rounded-md shadow overflow-hidden">
          ...
          <ul v-if="results.length" class="bg-white border rounded shadow border-gray-200 pr-6 w-full mt-2 max-h-52 overflow-y-auto">
            <li v-for="result in results" class="px-1 py-2 border-b border-gray-200 relative cursor-pointer hover:bg-gray-100 hover:text-gray-900">
              <div class="flex items-center">
                ...
                <div v-if="!result.added" class="self-end"><button @click="addTrack(result)" class="btn-indigo" type="button">{{ __("Add") }}</button></div>
                <div v-else class="self-end"><button class="btn-green" @click="removeTrack(result)" type="button">{{ __("Remove") }}</button></div>
              </div>
            </li>
          </ul>
        </div>
      ...
      <div class="mx-4">
        <table class="w-full whitespace-nowrap">
          ...
          <tr v-for="track in tracks.data" :key="track.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
            <td class="border-t">
