...
const filteredUsers = computed(() => {
  return users.value.filter((user) => user.name.toLowerCase().includes(mentionFilter.value.toLowerCase()))
})
...
onUnmounted(() => {
  ...
  Echo.private(channel.value).stopListening('NewMessage').stopListening('MessageReported').stopListening('MessageDeleted')
})
...
<template>
  <div class="chat-panel" :class="{ 'chat-panel--embedded': embedded }">
    <div v-if="!embedded" class="chat-header">
      ...
      <button v-if="!reported" type="button" class="room-action-btn !py-1.5 !text-[10px]" @click="alertingModerators = true" :title="__('Report a problem on the chat')">
      ...
      <div v-else class="flex items-center text-xs text-brand-accent">
    ...
    <div v-else class="chat-header chat-header--embedded">
      <button v-if="!reported" type="button" class="chat-icon-btn !h-7 !w-7" @click="alertingModerators = true" :title="__('Report a problem on the chat')">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
        </svg>
      </button>
      <div v-else class="flex items-center text-[10px] text-brand-accent">
    ...
    <div ref="messagesContainer" class="chat-messages flex flex-1 gap-1 overflow-y-auto px-2 py-2" :class="embedded ? 'flex-col' : 'flex-col-reverse'">
    ...
    <div v-if="user?.is_guest" class="chat-composer flex w-full">
      <div class="chat-guest-banner text-center">
        <Link :href="route('guest.to-register')" class="font-medium text-brand-accent underline hover:text-brand-accent-hover">{{ __('Register') }}</Link
        >{{ __(' or ') }}<Link :href="route('guest.to-login')" class="font-medium text-brand-accent underline hover:text-brand-accent-hover">{{ __('Login') }}</Link
        >{{ __('to chat with other players') }}
      </div>
    </div>

    <div v-else class="chat-composer relative flex w-full">
      <form @submit.prevent="sendMessage" class="relative flex w-full text-sm" role="form" aria-label="Message form">
        <div class="chat-input-wrap">
          <TextInput ref="inputRef" v-model="body" autocomplete="off" :inputClass="['border-0', 'focus:ring-0', 'focus:ring-offset-0', 'focus:border-transparent', 'focus:outline-none', 'bg-transparent', 'text-base', embedded ? 'py-2' : 'py-3', 'pl-4', 'pr-12']" class="flex-grow" @click="showEmojiPicker = false" @input="handleInput" :placeholder="__('Type your message...')" aria-label="Message input" aria-describedby="message-char-count" maxlength="500" />
          <button type="button" class="absolute right-2 p-1.5 text-white/50 transition-colors duration-200 hover:text-brand-accent focus:outline-none" @click="showEmojiPicker = !showEmojiPicker" aria-label="Open emoji picker" aria-expanded="showEmojiPicker" aria-controls="emoji-picker">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
              <path stroke-linecap="round" stroke-linejoin="round" d="M15.182 15.182a4.5 4.5 0 0 1-6.364 0M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0ZM9.75 9.75c0 .414-.168.75-.375.75S9 10.164 9 9.75 9.168 9 9.375 9s.375.336.375.75Zm-.375 0h.008v.015h-.008V9.75Zm5.625 0c0 .414-.168.75-.375.75s-.375-.336-.375-.75.168-.75.375-.75.375.336.375.75Zm-.375 0h.008v.015h-.008V9.75Z" />
            </svg>
          </button>
        </div>
        <div id="message-char-count" class="sr-only" aria-live="polite">{{ body.length }} characters remaining out of 500</div>
        <button type="submit" class="chat-send-btn" :disabled="!body.trim()" aria-label="Send message">
      ...
      <EmojiPicker
        ...
        class="retro-dropdown-panel absolute right-2 z-50 border border-white/10 bg-brand-deep shadow-xl"
