...
const segmentStartPercentage = computed(() => (segmentStart.value / audioDuration.value) * 100)

const segmentWidthPercentage = computed(() => (30 / audioDuration.value) * 100)

const currentTimeFormatted = computed(() => formatTime(currentTime.value))

const segmentStartFormatted = computed(() => formatTime(segmentStart.value))

const segmentEndFormatted = computed(() => formatTime(segmentStart.value + 30))

const audioDurationFormatted = computed(() => formatTime(audioDuration.value))
...
async function extract30sSegment(file, start, duration = 30) {
  try {
    ...
    const newBuffer = audioContext.createBuffer(audioBuffer.numberOfChannels, durationSamples, sampleRate)
...
<template>
  <div>
    <button class="flex w-full items-center justify-center gap-2 rounded-lg border border-neutral-600 bg-neutral-700/50 px-4 py-2.5 text-sm font-medium text-neutral-200 transition-all duration-200 hover:border-teal-500/50 hover:bg-neutral-700 hover:text-white" @click="uploadingTrack = true">
    ...
    <Modal :show="uploadingTrack" @close="close" max-width="2xl">
      <div class="rounded-lg bg-neutral-900 p-6">
        <div class="mb-6">
          <h1 class="mb-2 text-2xl font-bold text-white">{{ __('Upload a song from your computer') }}</h1>
          <p class="mb-3 text-sm text-neutral-400">{{ __('Import an MP3 file from your computer. You can select a 30-second segment to use in the game') }}</p>
          <p class="text-xs text-neutral-500">{{ __('All fields are required. You can find MP3 files at') }} <a href="https://emp3juice.la/" target="_blank" class="text-teal-400 underline transition-colors hover:text-teal-300">emp3juice.la</a></p>
        </div>

        <form @submit.prevent="uploadTrack" class="space-y-6">
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <TextInput v-model="form.artist_name" label="Nom de l'artiste" id="artist_name" :error="form.errors.artist_name" required class="w-full" />
            <TextInput v-model="form.track_name" label="Titre de la chanson" id="track_name" :error="form.errors.track_name" required class="w-full" />
          </div>

          <div class="rounded-lg border border-neutral-700 bg-neutral-800/50 p-4">
            <FileInput v-model="form.audio" label="Audio" id="audio" accept="audio/mp3" :error="form.errors.audio" required />
            <p class="mt-2 flex items-center text-sm text-red-400">
          ...
          <div v-if="form.audio" class="space-y-4 rounded-lg border border-neutral-700 bg-neutral-800/50 p-5">
            <h3 class="mb-3 text-lg font-medium text-white">Sélection de l'extrait</h3>
            <div class="flex items-center gap-4">
              <button type="button" @click="playPreview" class="flex h-12 w-12 items-center justify-center rounded-full bg-neutral-700 shadow-lg transition-colors hover:bg-neutral-600 hover:shadow-neutral-500/20" :disabled="!audioPreview" :class="{ 'opacity-50': !audioPreview }">
                <svg v-if="!isPlaying" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-6 w-6 text-white">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M5.25 5.653c0-.856.917-1.398 1.667-.986l11.54 6.348a1.125 1.125 0 010 1.971l-11.54 6.347a1.125 1.125 0 01-1.667-.985V5.653z" />
                </svg>
                <svg v-else xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-6 w-6 text-white">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 5.25v13.5m-7.5-13.5v13.5" />
                </svg>
              </button>

              <!-- Waveform Container -->
              <div ref="waveformContainer" class="relative h-20 w-full flex-1 cursor-pointer overflow-hidden rounded-xl bg-neutral-900 shadow-inner" @click="handleWaveformClick">
                <!-- Loading Indicator -->
                <div v-if="isGeneratingWaveform" class="absolute inset-0 z-10 flex items-center justify-center bg-neutral-800/70 backdrop-blur-sm">
                  <div class="flex flex-col items-center">
                    ...
                    <span class="mt-2 text-xs text-white">Génération de la forme d'onde...</span>
                  </div>
                </div>

                <!-- Waveform -->
                <div class="pointer-events-none absolute inset-0 flex h-full w-full items-end gap-[1px]">
                  <div v-for="(value, index) in waveformData" :key="index" class="flex-1 rounded-full bg-gradient-to-t from-neutral-500/80 to-neutral-400/50 transition-all duration-300" :class="{ 'animate-pulse': isGeneratingWaveform }" :style="{ height: `${value * 100}%` }"></div>
                </div>

                <!-- Selection Window (orange/yellow highlight, draggable) -->
                <div
                  class="absolute inset-y-0 cursor-grab border-2 border-yellow-400 bg-yellow-300/20 transition-all duration-100 active:cursor-grabbing"
                  :style="{
                ...
                <!-- Start Handle -->
                <div class="absolute bottom-0 top-0 w-1 cursor-ew-resize bg-yellow-400" :style="{ left: `${segmentStartPercentage}%` }" @mousedown.stop="handleDragStart('start', $event)" @touchstart.stop="handleDragStart('start', $event)">
                  <div class="absolute -left-1.5 -top-1 h-4 w-4 rounded-full border-2 border-white bg-yellow-400 shadow-lg"></div>
                </div>

                <!-- End Handle (not draggable, just visual) -->
                <div class="absolute bottom-0 top-0 w-1 bg-yellow-400" :style="{ left: `${segmentStartPercentage + segmentWidthPercentage}%` }">
                  <div class="absolute -left-1.5 -top-1 h-4 w-4 rounded-full border-2 border-white bg-yellow-400 shadow-lg"></div>
                </div>

                <!-- Playhead -->
                <div v-if="isPlaying" class="absolute bottom-0 top-0 w-0.5 bg-white shadow-[0_0_5px_rgba(255,255,255,0.5)]" :style="{ left: `${(currentTime / audioDuration) * 100}%` }"></div>

                <!-- Time Markers -->
                <div class="pointer-events-none absolute bottom-0 left-0 right-0 flex justify-between px-2 text-[10px] text-neutral-500">
                  <span>0:00</span>
            ...
            <div class="mt-2 flex justify-between rounded-lg bg-neutral-900/50 p-2 text-xs text-neutral-400">
          ...
          <div class="rounded-lg border border-neutral-700 bg-neutral-800/50 p-4">
            <FileInput v-model="form.artwork" label="Pochette de l'album" id="artwork" :error="form.errors.artwork" accept="image/*" required />
            <p class="mt-2 flex items-center text-sm text-red-400">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="mr-1 h-4 w-4">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
              </svg>
            ...
            <div v-if="artworkPreview" class="mt-4 flex items-center gap-3">
              <div class="relative h-24 w-24 overflow-hidden rounded-lg border border-neutral-600 shadow-lg">
                <img :src="artworkPreview" alt="Aperçu de la pochette" class="h-full w-full object-cover" />
              </div>
              <div class="text-sm text-neutral-400">
                <p class="mb-1 font-medium text-white">Aperçu de la pochette</p>
                <p>Cette image sera utilisée pour l'affichage de la chanson dans la playlist.</p>
              </div>
            </div>
          </div>

          <div v-if="form.errors.message" class="rounded-lg border border-red-500/50 bg-red-500/20 p-3 text-sm text-red-400">
            {{ form.errors.message }}
          </div>

          <div class="flex justify-end">
            <button type="submit" class="flex transform items-center gap-2 rounded-lg bg-gradient-to-r from-neutral-600 to-neutral-500 px-6 py-3 text-sm font-medium text-white shadow-lg transition-all duration-200 hover:translate-y-[-2px] hover:from-neutral-500 hover:to-neutral-400 hover:shadow-neutral-500/20 disabled:transform-none disabled:cursor-not-allowed disabled:opacity-50" :disabled="form.processing || processingAudio">
              <svg v-if="form.processing || processingAudio" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-2 size-4 animate-spin">
                <path d="M21 12a9 9 0 1 1-6.219-8.56" />
              </svg>
