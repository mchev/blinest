...
<template>
  <a v-if="goal && donationUrl" :href="donationUrl" target="_blank" rel="noopener noreferrer external nofollow" class="group flex w-full items-center gap-2.5 rounded-lg border border-white/10 bg-brand-deep/80 px-3 py-2 text-left transition hover:border-rose-400/30 hover:bg-brand-deep" :aria-label="__('Donation mobile bar aria')" data-umami-event="Faire un don">
    ...
    <span class="shrink-0 whitespace-nowrap text-[11px] font-semibold tabular-nums text-white/80">
      {{ formatEuros(goal.effective_cents ?? goal.raised_cents) }}
      <span class="text-white/40">/</span>
      {{ formatEuros(goal.goal_cents) }}
    </span>
