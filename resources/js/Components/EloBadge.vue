...
<template>
  ...
  <div
    ...
    :class="[
      'inline-flex items-center gap-1.5 rounded-lg border font-semibold shadow-md transition-all',
      `bg-gradient-to-r ${eloInfo.bgGradientDark}`,
      sizeConfig.padding,
      clickable ? 'cursor-pointer hover:scale-105 hover:shadow-lg' : '',
      // Border colors by rank
      eloInfo.name === 'Bronze' ? 'border-amber-600/60' : eloInfo.name === 'Silver' ? 'border-slate-400/60' : eloInfo.name === 'Gold' ? 'border-yellow-400/60' : eloInfo.name === 'Platinum' ? 'border-teal-400/60' : eloInfo.name === 'Diamond' ? 'border-blue-400/60' : eloInfo.name === 'Master' ? 'border-purple-500/60' : 'border-pink-500/60',
    ]"
  ...
  >
    ...
    <span :class="[sizeConfig.numberSize, 'font-extrabold text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]']">{{ elo }}</span>
    <span v-if="showChange && eloChange !== null" :class="[sizeConfig.numberSize, 'font-bold drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]', eloChange > 0 ? 'text-green-100' : eloChange < 0 ? 'text-red-100' : 'text-white/90']"> {{ eloChange > 0 ? '+' : '' }}{{ eloChange }} </span>
  </div>
  ...
  <div
    ...
    :class="[
      'inline-flex items-center justify-center rounded-md border font-bold shadow-sm',
  ...
  <div v-else-if="variant === 'minimal'" :class="['inline-flex items-center gap-1', eloInfo.textColorMinimal, clickable ? 'cursor-pointer hover:opacity-90' : '']" :title="`${eloInfo.name} ${eloInfo.rank} - ${elo} ELO`" @click="handleClick" role="status" :aria-label="`${eloInfo.name} ${eloInfo.rank} - ${elo} ELO`">
  ...
  >
    <div class="flex items-center gap-1.5">
      <svg v-if="eloInfo.name === 'Grandmaster'" :class="[sizeConfig.iconSize, 'text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]']" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" />
      </svg>
      <svg v-else :class="[sizeConfig.iconSize, 'text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]']" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
