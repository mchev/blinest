  ...
  <div class="md:flex md:flex-shrink-0">
    <div class="flex w-full items-center justify-between px-4 py-2 md:flex-shrink-0 md:px-12">
      ...
      <div class="flex items-center justify-end">
        <button type="button" @click="showingSearchbar = !showingSearchbar" class="lg:hidden"><Icon name="search" class="mr-4 h-5 w-5" /></button>
        <Link v-if="user" :href="route('rankings.index')" :title="__('Rankings')"><Icon name="podium" class="mr-4 h-5 w-5" /></Link>
        <a href="https://discord.com/invite/uKyVgcxcFa" target="_blank" :title="__('Join the Blinest community on Discord')" class="umami--click--discord-button mr-4">
