...
watch(() => props.user.bookmarks, (newBookmarks) => {
  if (newBookmarks) {
...
<template>
  <div class="space-y-3">
    <div v-for="room in bookmarks" :key="room.id" class="flex items-center gap-4 squircle-nested-sm border border-neutral-700/50 bg-neutral-800/50 px-4 py-3 transition-colors hover:bg-neutral-800/70">
      <Link class="flex items-center" :href="route('rooms.show', room.slug)">
        <img v-if="room.photo" :src="room.photo" class="h-12 w-12 squircle-nested-xs object-cover" loading="lazy" />
    ...
    <div v-if="!isExpanded && bookmarks.length > 0 && bookmarks.length < (user.bookmarks?.total || 0)" class="flex justify-center pt-4">
      <button @click="expandSection" class="squircle-nested-xs border border-neutral-600 bg-neutral-700 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-neutral-600">
        {{ __('View all') }} ({{ user.bookmarks?.total || 0 }})
      </button>
    </div>
