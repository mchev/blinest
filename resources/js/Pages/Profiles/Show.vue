...
<template>
  <AppLayout>
        ...
        <Card>
          <div class="flex flex-col items-center text-center">
            ...
            <Link v-if="user.team" :href="route('teams.show', user.team.id)" class="group mb-4 block w-full max-w-[280px] rounded-xl border border-violet-500/30 bg-gradient-to-br from-violet-950/50 via-neutral-900/80 to-fuchsia-950/30 p-3 text-left shadow-[0_0_24px_-8px_rgba(139,92,246,0.35)] transition hover:border-violet-400/45 hover:shadow-[0_0_32px_-6px_rgba(139,92,246,0.45)]">
              <div class="flex items-center gap-3">
                <div class="relative shrink-0">
                  ...
                  <img v-if="user.team.photo" :src="user.team.photo" :alt="user.team.name" class="relative h-12 w-12 rounded-full object-cover ring-2 ring-violet-400/50" loading="lazy" />
                  <div v-else class="relative flex h-12 w-12 items-center justify-center rounded-full bg-neutral-800 text-lg font-black text-violet-300 ring-2 ring-violet-400/40">
                ...
                <div class="min-w-0 flex-1">
                  ...
                  <p class="text-xs text-neutral-500 group-hover:text-neutral-400">{{ __('View team page') }} →</p>
                </div>
              </div>
            </Link>
            ...
            <div class="mb-6 grid w-full grid-cols-2 gap-3">
              ...
              <div class="squircle-nested flex flex-col items-center justify-center border border-neutral-700/50 bg-neutral-800/50 p-4">
                <EloBadge :elo="user.elo" size="md" variant="compact" />
                <div class="mt-1 text-xs text-neutral-400">{{ __('ELO') }}</div>
              </div>
            </div>
          ...
          <div class="relative h-48">
            <div v-if="loadingEvolution" class="absolute inset-0 flex items-end justify-between px-4 pb-4">
              <div class="flex w-full items-end gap-1">
                <div class="h-8 w-full animate-pulse rounded-t bg-neutral-700/50" style="animation-delay: 0s"></div>
                <div class="h-12 w-full animate-pulse rounded-t bg-neutral-700/50" style="animation-delay: 0.1s"></div>
                <div class="h-16 w-full animate-pulse rounded-t bg-neutral-700/50" style="animation-delay: 0.2s"></div>
                <div class="h-20 w-full animate-pulse rounded-t bg-neutral-700/50" style="animation-delay: 0.3s"></div>
                <div class="h-14 w-full animate-pulse rounded-t bg-neutral-700/50" style="animation-delay: 0.4s"></div>
                <div class="h-10 w-full animate-pulse rounded-t bg-neutral-700/50" style="animation-delay: 0.5s"></div>
                <div class="h-16 w-full animate-pulse rounded-t bg-neutral-700/50" style="animation-delay: 0.6s"></div>
              </div>
            </div>
    ...
    <LevelModal :show="showLevelModal" :level="user.level" :current-xp="user.current_xp" :xp-for-next-level="user.xp_for_next_level" :total-xp="user.total_xp" :level-metrics="user.level_metrics" @close="showLevelModal = false" />
  </AppLayout>
</template>
