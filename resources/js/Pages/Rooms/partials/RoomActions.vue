  ...
  <div class="room-actions-bar">
    <div class="room-actions">
      <button type="button" class="room-action-btn" :aria-label="__('Share this room')" @click="showShareModal = true">
        ...
        <span class="hidden md:inline">{{ __('Share') }}</span>
      </button>
      ...
      <button v-if="user" type="button" @click="showPodiumModal = true" :aria-label="__('Show rankings for this room')" class="room-action-btn room-action-btn--highlight">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-4 w-4 shrink-0">
          <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 18L9 11.25l4.306 4.307a11.95 11.95 0 015.814-5.519l2.74-1.22m0 0l-5.94-2.28m5.94 2.28l-2.28 5.941" />
        </svg>
        <span class="hidden md:inline">{{ __('Rankings') }}</span>
      </button>

      <Bookmark :room="room" />

      <button v-if="user && room.is_chat_active" type="button" class="room-action-btn" :class="{ 'room-action-btn--active': showSidebar }" @click="toggleChat" :aria-label="showSidebar ? __('Hide chatbox') : __('Show chatbox')" :aria-pressed="showSidebar">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
        </svg>
        <span class="hidden md:inline">{{ __('Chat') }}</span>
      </button>
