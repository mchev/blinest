    ...
    <button type="button" class="room-track-card__toggle w-full text-left" :aria-expanded="expanded" @click="toggleExpanded">
      <div class="flex items-center gap-2 p-2">
        ...
        <div v-if="user" class="flex shrink-0 items-center gap-1" @click.stop>
          <button type="button" class="room-track-vote-btn" :class="{ 'room-track-vote-btn--up-active': track.user_voted_up }" :title="__('Upvote')" :aria-label="__('Upvote this track')" :aria-pressed="track.user_voted_up" @click="voteTrackUp(track)">
            <Icon name="thumb-up" class="h-3 w-3" aria-hidden="true" />
            <span>{{ track.upvotes }}</span>
          </button>
          <button type="button" class="room-track-vote-btn" :class="{ 'room-track-vote-btn--down-active': track.user_voted_down }" :title="__('Downvote')" :aria-label="__('Downvote this track')" :aria-pressed="track.user_voted_down" @click="voteTrackDown(track)">
