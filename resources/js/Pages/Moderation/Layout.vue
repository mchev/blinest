        ...
        <div :class="['fixed inset-y-0 left-0 z-40 w-72 transform border-r border-white/10 bg-brand-deep backdrop-blur-sm transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0', isSidebarOpen ? 'translate-x-0' : '-translate-x-full']">
          <div class="flex h-full flex-col">
            ...
            <nav class="flex-1 space-y-2 p-4">
              <Link v-for="item in navigation" :key="item.name" :href="item.href" class="group relative flex items-center rounded-xl px-4 py-3 text-sm font-medium transition-all duration-200 ease-in-out" :class="[route().current(item.routeName) ? 'border border-brand-accent/30 bg-brand-accent/10 font-bold text-brand-accent' : 'text-white/60 hover:bg-brand-midnight hover:text-white']">
                <div class="relative z-10 flex w-full items-center">
                  ...
                  <span v-if="route().current(item.routeName)" class="ml-2 h-2 w-2 bg-brand-accent"></span>
                </div>
              </Link>
            </nav>
          </div>
          ...
          <div class="retro-panel p-6">
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-2">
                <span class="border border-brand-accent/30 bg-brand-accent/10 px-3 py-1 text-xs font-medium uppercase tracking-wider text-brand-accent"> Modération </span>
              </div>
            </div>
          </div>
