  ...
  <AppLayout>
    <div class="mx-auto max-w-5xl space-y-8 px-4 py-8">
      ...
      <div class="space-y-4 text-center">
        ...
        <p class="text-lg text-neutral-400">Définitions claires et précises des termes utilisés sur la plateforme</p>
      </div>
      ...
      <div class="rounded-2xl border border-slate-700 bg-gradient-to-br from-slate-800 to-slate-900 p-6">
        <div class="mb-4 flex items-start justify-between">
          ...
          <span class="rounded bg-slate-800/50 px-3 py-1 text-xs text-neutral-500"> Mis à jour le {{ new Date().toLocaleDateString('fr-FR', { year: 'numeric', month: 'long', day: 'numeric' }) }} </span>
        </div>
        <p class="mb-4 text-neutral-300">Ce glossaire définit de manière claire et assertive tous les termes techniques et concepts utilisés sur Blinest. Chaque définition est structurée pour être facilement compréhensible et extractible par les moteurs de recherche génératifs.</p>
        <p class="text-neutral-300"><strong>Blinest est une plateforme de quiz musicaux multijoueurs en ligne</strong> où les joueurs s'affrontent dans des blind tests organisés en rooms (salles de jeu). La plateforme utilise trois systèmes de progression distincts : les scores (activité), les levels (engagement) et l'ELO (compétence).</p>
      </div>

      <!-- Definitions List -->
      <div class="space-y-6">
        <div v-for="(def, index) in definitions" :key="index" class="rounded-2xl border border-slate-700 bg-gradient-to-br from-slate-800 to-slate-900 p-6">
          <h2 class="mb-3 text-2xl font-bold text-white">{{ def.term }}</h2>
          <p class="mb-3 text-lg text-neutral-300">
            {{ def.definition }}
          </p>
          <div v-if="def.context" class="mb-3 rounded-lg bg-slate-800/50 p-4">
            <p class="text-sm text-neutral-300"><strong class="text-white">Contexte Blinest :</strong> {{ def.context }}</p>
          </div>
          <div v-if="def.related && def.related.length" class="flex flex-wrap gap-2">
            <span class="text-xs text-neutral-400">Voir aussi :</span>
            <span v-for="(term, termIndex) in def.related" :key="termIndex" class="rounded bg-slate-700/50 px-2 py-1 text-xs text-neutral-300">
              {{ term }}
            </span>
          </div>
        </div>
      ...
      <!-- Quick Facts -->
      <div class="rounded-2xl border border-indigo-700/50 bg-gradient-to-br from-indigo-900/20 to-indigo-800/20 p-6">
        <h2 class="mb-4 text-2xl font-bold text-indigo-400">Facts clés sur Blinest</h2>
