...
<template>
  <Link :rel="(!room.is_public) ? 'nofollow': ''" :href="`/rooms/${room.slug}`" class="relative flex h-52 w-full flex-col items-center justify-center rounded-md bg-neutral-800 bg-contain bg-center transition duration-100 ease-in-out hover:rotate-1 p-1" :style="`background-image: url(${room.photo});`" :title="room.name">
    <article class="relative h-full w-full">
      ...
      <div class="absolute bottom-0 flex w-full items-center justify-between gap-2 bg-neutral-900 py-2 uppercase text-gray-100">
