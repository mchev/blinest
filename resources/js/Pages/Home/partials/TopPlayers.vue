...
<template>
  <article v-if="list && list.length">
    <h3 v-if="!embedded" class="retro-title retro-title--secondary mb-5 text-center text-sm">
    ...
    <ul v-if="rest.length" class="flex flex-col gap-2">
      <li v-for="(score, index) in rest" :key="score.user?.id || index + 4" class="flex items-center gap-3 rounded-lg border border-white/10 bg-brand-deep px-3 py-2 transition hover:border-white/20 hover:bg-brand-deep-hover">
        ...
        <div class="min-w-0 flex-1">
          <Link v-if="score.user?.id" :href="route('user.profile', { user: score.user.id })" class="block truncate text-sm font-medium text-white hover:text-white/80">
            {{ score.user.name }}
          </Link>
