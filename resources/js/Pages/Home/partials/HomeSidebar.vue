...
<template>
  <aside class="w-full shrink-0 lg:w-80 xl:w-96">
    <div class="lg:sticky lg:top-6 lg:self-start">
      <div class="overflow-hidden rounded-xl border border-zinc-800 bg-arena-panel shadow-xl shadow-black/30">
        <HomeSidebarSection
          v-for="featuredRoom in featuredRooms"
          :key="`featured-${featuredRoom.id}`"
          :kicker="__('Featured')"
        >
        ...
        <HomeSidebarSection :kicker="__('Play')" :title="__('Quick Links')">
          <div class="grid grid-cols-2 gap-2">
            <Link
              ...
              class="sidebar-tile group"
            >
              <span class="flex h-10 w-10 items-center justify-center rounded-lg bg-teal-500/20 text-teal-300 ring-1 ring-teal-500/30 transition group-hover:bg-teal-500/30" aria-hidden="true">
            ...
            <a
              ...
              class="sidebar-tile group"
              :title="__('Join the Blinest community on Discord')"
              data-umami-event="Discord button"
            >
              <span class="flex h-10 w-10 items-center justify-center rounded-lg bg-twitch/20 text-twitch ring-1 ring-twitch/30 transition group-hover:bg-twitch/30" aria-hidden="true">
            ...
            <Link
              ...
              class="sidebar-tile group"
            >
              <span class="flex h-10 w-10 items-center justify-center rounded-lg bg-amber-500/20 text-amber-300 ring-1 ring-amber-500/30 transition group-hover:bg-amber-500/30" aria-hidden="true">
                <Icon name="trophy" class="h-5 w-5" />
              </span>
              <span class="text-xs font-semibold text-white">{{ __('Rankings') }}</span>
            </Link>
            <a
              href="https://shop.blinest.com"
              target="_blank"
              rel="external nofollow"
              data-umami-event="Shop button sidebar"
              class="sidebar-tile group"
            >
              <span class="flex h-10 w-10 items-center justify-center rounded-lg bg-emerald-500/20 text-emerald-300 ring-1 ring-emerald-500/30 transition group-hover:bg-emerald-500/30" aria-hidden="true">
        ...
        <HomeSidebarSection :kicker="__('Support Blinest')" :border="false">
          ...
          <a
            ...
            target="_blank"
            rel="external nofollow"
            data-umami-event="Faire un don"
            class="game-btn-play-primary"
          >
            {{ __('Donate') }}
          </a>
        </HomeSidebarSection>
