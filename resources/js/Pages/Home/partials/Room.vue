...
<template>
  <Link :href="`/rooms/${room.id}`" class="relative flex h-48 w-full flex-col items-center justify-center rounded-md bg-neutral-800 bg-cover bg-center shadow shadow transition duration-100 ease-in-out hover:z-10 hover:scale-110 hover:grayscale-0" :style="`background-image: url(${room.photo_path || room.mosaic});`">
    <article class="relative h-full w-full">
      <div v-if="!room.is_public" class="ribbon truncate text-xs">@{{ room.owner.name }}</div>
      <div class="absolute top-0 left-0 w-auto rounded-br-md rounded-tl-md bg-neutral-700 py-1 px-2 text-sm ease-in-out hover:scale-110" :title="__('Players')">
        <div class="flex items-center">
          <span v-if="room.password" class="font-bold text-orange-400 mr-1">
