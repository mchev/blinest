...
const tabs = computed(() => {
  const items = [{ id: 'players', label: __('Players'), icon: 'trophy', route: route('rankings.index') }]

  if (user.value) {
    items.push({ id: 'minigames', label: __('Mini-games'), icon: 'gamepad', route: route('rankings.minigames') }, { id: 'teams', label: __('Teams'), icon: 'users', route: route('rankings.teams') })
  }
