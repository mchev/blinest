  ...
  <AppLayout>
    <section>
      <div class="mx-auto max-w-5xl px-4 py-4 sm:py-8">
        ...
        <!-- User Position -->
        <UserPosition v-if="userPosition" :position="userPosition" :score="typeof userTeamScore === 'string' ? parseFloat(userTeamScore) : userTeamScore" type="teams" />

        <!-- Ranking List -->
        <div v-if="topTeams && topTeams.data && topTeams.data.length > 0" class="mt-6">
          <RankingList :items="topTeams" type="teams" />
          <Pagination :links="topTeams.links" />
        </div>
