  ...
  <AppLayout>
    <section>
      <div class="mx-auto max-w-5xl px-4 py-4 sm:py-8">
        ...
        <!-- User Position -->
        <UserPosition v-if="userPosition" :position="userPosition" :score="typeof userWeekScore === 'string' ? parseFloat(userWeekScore) : userWeekScore" type="week" />

        <!-- Ranking List -->
        <div v-if="topWeek && topWeek.data && topWeek.data.length > 0" class="mt-6">
          <RankingList :items="topWeek" type="week" />
          <Pagination :links="topWeek.links" />
        </div>
