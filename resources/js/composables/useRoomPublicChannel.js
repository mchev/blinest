...
export function useRoomPublicChannel(room, rootRef) {
    ...
    const progressPercent = computed(() =>
        Math.min(100, Math.round((currentTrackIndex.value / tracksByRound.value) * 100))
    )

    const tracksRemaining = computed(() =>
        Math.max(0, tracksByRound.value - currentTrackIndex.value)
    )

    const isNearEnd = computed(() =>
        isPlaying.value && tracksRemaining.value > 0 && tracksRemaining.value <= 2
    )
    ...
    const visibleTrackDots = computed(() =>
        Math.min(tracksByRound.value, 12)
    )
