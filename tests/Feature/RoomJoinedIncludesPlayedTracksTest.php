...
class RoomJoinedIncludesPlayedTracksTest extends TestCase
{
    ...
    public function test_joined_includes_timing_payload_and_start_time_for_mid_track_join(): void
    {
        $category = Category::create(['name' => 'Cat']);
        $owner = User::factory()->create();
        $player = User::factory()->create();
        ...
        $response->assertOk();
        $response->assertJsonPath('track.id', $track->id);
        $response->assertJsonPath('round.current_track_started_at', $startedAt->toIso8601String());
        $response->assertJsonPath('round.track_duration', 30);
        $this->assertGreaterThanOrEqual(11, (int) $response->json('startTime'));
        $this->assertLessThanOrEqual(13, (int) $response->json('startTime'));
        $this->assertNull($response->json('interTrackPause'));
    }
}
