              ...
              if (selectedUser.value.bans) {
                ...
                if (selectedUser.value.bans.length) {
                  ...
                  ssrRenderList(selectedUser.value.bans, (ban) => {
                    _push2(`<li class="border-neutral-600-b border-neutral-600-neutral-600 mb-2 flex flex-col border border p-2"${_scopeId}><span class="text-xs text-neutral-500"${_scopeId}>Banni par : ${ssrInterpolate(ban.banned_by)}</span><span class="text-xs text-neutral-500"${_scopeId}>le : ${ssrInterpolate(ban.created_at)}</span><span class="text-xs text-neutral-500"${_scopeId}>Raison : ${ssrInterpolate(ban.comment)}</span><span class="text-xs text-neutral-500"${_scopeId}>Expire le : ${ssrInterpolate(ban.expired_at)}</span></li>`);
                  });
                  _push2(`<!--]--></ul>`);
                } else {
