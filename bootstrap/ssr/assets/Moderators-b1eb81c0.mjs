        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            ssrRenderList(__props.moderators, (room) => {
              _push2(`<li class="flex flex-col mb-4 border-b border-neutral-600"${_scopeId}>${ssrInterpolate(room.name)} <ul class="flex flex-wrap gap-1 mb-2"${_scopeId}><!--[-->`);
              ssrRenderList(room.moderators, (moderator) => {
                _push2(`<li${_scopeId}><span class="badge flex items-center gap-1"${_scopeId}><img${ssrRenderAttr("src", moderator.photo)} class="w-5 h-5 rounded-full"${_scopeId}>${ssrInterpolate(moderator.name)}</span></li>`);
              });
              _push2(`<!--]--></ul></li>`);
            });
            _push2(`<!--]--></ul>`);
              ...
              createVNode("ul", null, [
                (openBlock(true), createBlock(Fragment, null, renderList(__props.moderators, (room) => {
                  return openBlock(), createBlock("li", {
                    key: room.id,
                    class: "flex flex-col mb-4 border-b border-neutral-600"
                  }, [
                    createTextVNode(toDisplayString(room.name) + " ", 1),
                    createVNode("ul", { class: "flex flex-wrap gap-1 mb-2" }, [
