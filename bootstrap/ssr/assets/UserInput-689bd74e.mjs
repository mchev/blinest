    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(ssrRenderComponent(_sfc_main$2, mergeProps({
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="group mr-1 flex h-full cursor-pointer select-none items-center"${_scopeId}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-6 w-6"${_scopeId}><path stroke-linecap="round" stroke-linejoin="round" d="M19.114 5.636a9 9 0 010 12.728M16.463 8.288a5.25 5.25 0 010 7.424M6.75 8.25l4.72-4.72a.75.75 0 011.28.53v15.88a.75.75 0 01-1.28.53l-4.72-4.72H4.51c-.88 0-1.704-.507-1.938-1.354A9.01 9.01 0 012.25 12c0-.83.112-1.633.322-2.396C2.806 8.756 3.63 8.25 4.51 8.25H6.75z"${_scopeId}></path></svg></div>`);
          } else {
            return [
              createVNode("div", { class: "group mr-1 flex h-full cursor-pointer select-none items-center" }, [
...
const _sfc_main = /* @__PURE__ */ Object.assign(__default__, {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (message.value) {
        _push(`<blockquote class="${ssrRenderClass([{ "bg-teal-600": message.value.type === "good", "bg-orange-600": message.value.type === "almost", "bg-red-600": message.value.type === "bad" }, "absolute bottom-full right-0 flex translate-y-[-80%] translate-x-[-50%] items-center rounded-lg bg-teal-600 py-1 px-2 text-neutral-100"])}">${ssrInterpolate(message.value.body)} <div class="${ssrRenderClass([{ "border-t-teal-600": message.value.type === "good", "border-t-orange-600": message.value.type === "almost", "border-t-red-600": message.value.type === "bad" }, "absolute left-5 top-full mt-1 h-full h-0 w-full w-0 translate-y-[-50%] border-t-[10px] border-l-[10px] border-r-[10px] border-t-transparent border-l-transparent border-r-transparent"])}"></div></blockquote>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<input${ssrRenderAttr("value", text.value)} type="text" class="h-14 w-full flex-grow rounded-none rounded-bl-md border-none border-none p-2 text-2xl uppercase text-gray-600 focus:shadow-none focus:outline-none focus:ring-0" placeholder="Une idée?" autofocus${ssrIncludeBooleanAttr(inputDisabled.value) ? " readonly" : ""}>`);
      _push(ssrRenderComponent(_sfc_main$1, { class: "-ml-1 flex h-14 items-center justify-center bg-white p-2 text-neutral-700" }, null, _parent));
      _push(`<button type="submit" class="btn-send h-14"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="4" stroke="currentColor" class="h-6 w-6"><title>${ssrInterpolate(_ctx.__("Send"))}</title><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"></path></svg></button></div></form><ul class="mt-2 flex gap-2"><!--[-->`);
